...
#EXEC OBJ LOAD FILE=KF2CNF.utx PACKAGE=MutKF2Hud
#EXEC OBJ LOAD FILE=SXKEND.utx PACKAGE=MutKF2Hud
...
var transient KFWeapon OwnerWeapon;
var transient class<KFWeapon> OwnerWeaponClass;

//----------------------------------------------------------------------------

simulated function LinkActors()
{
    super(HUD).LinkActors();
    if(PawnOwner != none)
    {
...
{
    local class<KFVeterancyTypes> SV;
    ...
    {
        SV = KFPlayerReplicationInfo(PawnOwnerPRI).ClientVeteranSkill;
    }
    if(SV != none)
    {
...
simulated function DrawKFHUDTextElements(Canvas C)
{
	...
	CF = C.Font;
	C.Font = KF2Font;//original kf2Font
	C.FontScaleX = 1.0;
	C.FontScaleY = 1.0;
	...
	// C.Font = KFF;
	C.Font = KF2Font;//original kf2Fonts
...
function Texture GetPerkIcon(KFPlayerReplicationInfo PRI)
{   
    if (PRI.ClientVeteranSkillLevel <= 5)
        return PRI.ClientVeteranSkill.default.OnHUDIcon;

    else
        return PRI.ClientVeteranSkill.default.OnHUDGoldIcon;
}
...
simulated function DrawOldHudItems(Canvas C)
{
	...
	if (SV != None)
	{
		...
		TempMaterial = GetPerkIcon(KFPRI);
	...
	// -- WEAPON NAME -- //bDrawWeaponName && 
	if (OwnerWeaponClass != None && bDrawWeaponName)
	{
	...
	// -- WEIGHT TEXT -- //
	WS = int(SRHPawn.CurrentWeight)$"/"$int(SRHPawn.MaxCarryWeight);
    ...
    {
        if(Welder(OwnerWeapon) != none )//&& KF2Welder(OwnerWeapon) != none
        {
            C.FontScaleX = MagAmmoScale * SS;
            C.FontScaleY = MagAmmoScale * SS;
            C.Font = KF2Font;//original kf2Fonts
            C.TextSize(string(WelderDigits.Value), fx, FY);
            HDX = SX + ((MagAmmoX * KF2BLBase.TextureScale) * SS);
            HDY = SY + ((MagAmmoY * KF2BLBase.TextureScale) * SS);
            C.SetPos(HDX - fx, HDY - (FY * 0.10));
            C.DrawColor = KF2TextColor;
            C.DrawText(string(WelderDigits.Value));
        }
        else
        {
            if(Syringe(OwnerWeapon) != none)
            {
                ...
                C.FontScaleY = MagAmmoScale * SS;
                C.Font = KF2Font;//original kf2Fonts
                C.TextSize(string(SyringeDigits.Value), fx, FY);
                HDX = SX + ((MagAmmoX * KF2BLBase.TextureScale) * SS);
                HDY = SY + ((MagAmmoY * KF2BLBase.TextureScale) * SS);
                C.SetPos(HDX - fx, HDY - (FY * 0.10));
                C.DrawColor = KF2TextColor;
                C.DrawText(string(SyringeDigits.Value));
            }
            else
            {
                if(!OwnerWeaponClass.default.bMeleeWeapon && OwnerWeaponClass.default.bConsumesPhysicalAmmo)
                {
                    if((OwnerWeapon != none) && Huskgun(OwnerWeapon) != none)
                    {
                        C.SetPos(SX, SY);
                        C.DrawColor = KF2BRAngle.Tints[0];
                        C.DrawTile(KF2BRAngle.WidgetTexture, MX, MY, 0.0, 0.0, float(KF2BRAngle.WidgetTexture.MaterialUSize()), float(KF2BRAngle.WidgetTexture.MaterialVSize()));
                    ...
                    else
                    {
                        ...
                        C.FontScaleY = MagAmmoScale * SS;
                        C.Font = KF2Font;//original kf2Fonts
                        C.TextSize(string(MagAmmo), fx, FY);
                        HDX = SX + ((MagAmmoX * KF2BLBase.TextureScale) * SS);
                        HDY = SY + ((MagAmmoY * KF2BLBase.TextureScale) * SS);
                        C.SetPos(HDX - fx, HDY - (FY * 0.50));
                        C.DrawColor = KF2TextColor;
                        C.DrawText(string(MagAmmo));
                    }
                    if((PawnOwner != none) && PawnOwner.Weapon != none)
                    {
                        WS = string(int(CurClipsPrimary));
                ...
                else
                {
                    ...
                    C.DrawTile(KF2BRMelee.WidgetTexture, MX, MY, 0.0, 0.0, float(KF2BRMelee.WidgetTexture.MaterialUSize()), float(KF2BRMelee.WidgetTexture.MaterialVSize()));
                }
            }
        }
        if((OwnerWeapon != none) && KFMedicGun(OwnerWeapon) != none)
        {
            C.SetPos(SX, SY);
            C.DrawColor = KF2BRBase.Tints[0];
            C.DrawTile(KF2AltBase.WidgetTexture, MX, MY, 0.0, 0.0, float(KF2AltBase.WidgetTexture.MaterialUSize()), float(KF2AltBase.WidgetTexture.MaterialVSize()));
            C.SetPos(SX, SY);
            C.DrawColor = KF2AltScan.Tints[0];
            C.DrawTile(KF2AltScan.WidgetTexture, MX, MY, 0.0, 0.0, float(KF2AltScan.WidgetTexture.MaterialUSize()), float(KF2AltScan.WidgetTexture.MaterialVSize()));
            FMMat = GetSecondaryTex();
            HDX = SX + (((AltIconX - (AltIconSize * 0.50)) * KF2BLBase.TextureScale) * SS);
            HDY = SY + (((AltIconY - (AltIconSize * 0.50)) * KF2BLBase.TextureScale) * SS);
            C.SetPos(HDX, HDY);
            C.DrawColor = KF2TextColor;
            C.DrawTile(FMMat, (AltIconSize * KF2BLBase.TextureScale) * SS, (AltIconSize * KF2BLBase.TextureScale) * SS, 0.0, 0.0, float(FMMat.MaterialUSize()), float(FMMat.MaterialVSize()));
            C.FontScaleX = AltAmmoScale * SS;
            C.FontScaleY = AltAmmoScale * SS;
            C.Font = KF2Font;//original kf2Fonts
            C.TextSize(string(AltAmmoValue), fx, FY);
            HDX = SX + ((AltAmmoX * KF2BLBase.TextureScale) * SS);
            HDY = SY + ((AltAmmoY * KF2BLBase.TextureScale) * SS);
            C.SetPos(HDX - fx, HDY - (FY * 0.50));
            C.DrawColor = KF2TextColor;
            C.DrawText(string(AltAmmoValue));
        }

        
        if(((OwnerWeapon != none) && OwnerWeapon.bHasSecondaryAmmo) || ShowSecondary(OwnerWeapon))
        {
            if((Welder(OwnerWeapon) != none) || Syringe(OwnerWeapon) != none)
            {
                return;
            }
            C.SetPos(SX, SY);
...
function DisplayMessages(Canvas C)
{
    ...
    for( i=0; i<MessageCount; i++ )
    {
        ...
        YYL = 0;
        XXL = 0;
		TextMessages[i].Text = class'Helper'.static.ParseTags(TextMessages[i].Text);
...
static function SetFont(Canvas C, optional float Scale)
{
    if(Scale <= 0.0)
    {
        Scale = 0.50;
    }
    C.Font = Font'MutKF2Hud.KF2CNF';
    C.FontScaleX = Scale * 1.0;
    C.FontScaleY = Scale * 1.0;
}
...
simulated function DrawMessage( Canvas C, int i, float PosX, float PosY, out float DX, out float DY )
{
	...
	if (LocalMessages[i].Message != Class'PickupMessagePlus')
	{
		...
		if ( string(LocalMessages[i].Message) ~= "BHKillsMessage.BHKillsMessage" ||
			string(LocalMessages[i].Message) ~= "SXKServerA.SXKKillMSG" ||
			string(LocalMessages[i].Message) ~= "MutKillMessage.NKillsMessage" ||
			string(LocalMessages[i].Message) ~= "CombKillMSG.KillMSG" ||
			string(LocalMessages[i].Message) ~= "CombKillMSG.HSKillMSG" ||
			string(LocalMessages[i].Message) ~= "ComboKillMSG.KillMSG" ||
			string(LocalMessages[i].Message) ~= "ComboKillMSG.SKKillMSG" ||
			string(LocalMessages[i].Message) ~= "ComboKillMSG.HSKillMSG" ||
			...
			LocalMessages[i].Message == Class'KF2KillMessage')
		{
			...
			C.TextSize(class'Helper'.static.ParseTags(LocalMessages[i].StringMessage), FSX, FSY);
			// PosX = 1.0 - PosX;
			PosX = PosX;
			// PosX = 0.785; //right 
			SetFont(C, 0.425);
			DrawMessageB(C, i, PosX, PosY, DX, DY);
			return;
		}
		
		// Is this a damage message?
		if ( string(LocalMessages[i].Message) ~= "SXKServerA.SXKDmgMSG" ||
			string(LocalMessages[i].Message) ~= "CombKillMSG.DmgMSG" ||
			string(LocalMessages[i].Message) ~= "ComboKillMSG.DmgMSG" ||
		...
		// Is this a comboDmg msg?
		else if ( String(LocalMessages[i].Message) ~= "ComboKillMSG.ComboKillMSG")
		{
			SetFont(C, 0.625);
			DrawMessageB(C, i, PosX, PosY, DX, DY);
			return;
		...
		// Is this a Patkill Msg?
		else if ( string(LocalMessages[i].Message) ~= "BossCrusherM.BossCrusherMessage")
		{
			SetFont(C, 0.60);
			DrawMessageB(C, i, PosX, PosY, DX, DY);
			return;
		...
		// Is this a Slotmachine Msg?
		else if (string(LocalMessages[i].Message) ~= "SXKMachine.AdrenalineMessage" || 
				 string(LocalMessages[i].Message) ~= "SXKMachine.AmmoMessage" || 
				 string(LocalMessages[i].Message) ~= "SXKMachine.ArmorMessage" || 
				 string(LocalMessages[i].Message) ~= "SXKMachine.CatWomanMessage" || 
				 string(LocalMessages[i].Message) ~= "SXKMachine.DrunkMessage" || 
				 string(LocalMessages[i].Message) ~= "SXKMachine.GeoViewMessage" || 
				 string(LocalMessages[i].Message) ~= "SXKMachine.GodModeMessage" || 
				 string(LocalMessages[i].Message) ~= "SXKMachine.HatMessage" || 
				 string(LocalMessages[i].Message) ~= "SXKMachine.HeadMessage" || 
				 string(LocalMessages[i].Message) ~= "SXKMachine.HHGrenadeMessage" || 
				 string(LocalMessages[i].Message) ~= "SXKMachine.InstaDeathMessage" || 
				 string(LocalMessages[i].Message) ~= "SXKMachine.MaskMessage" || 
				 string(LocalMessages[i].Message) ~= "SXKMachine.PipeSelfDestMessage" || 
				 string(LocalMessages[i].Message) ~= "SXKMachine.RadarMessage" || 
				 string(LocalMessages[i].Message) ~= "SXKMachine.StarmanMessage" || 
				 string(LocalMessages[i].Message) ~= "SXKMachine.ToxicMessage" || 
				 string(LocalMessages[i].Message) ~= "SXKMachine.UHHGrenadeMessage" || 
				 string(LocalMessages[i].Message) ~= "SXKMachine.ZedDeathMessage" || 
				 string(LocalMessages[i].Message) ~= "BloodyHellMachine.AdrenalineMessage" || 
				 string(LocalMessages[i].Message) ~= "BloodyHellMachine.AmmoMessage" || 
				 string(LocalMessages[i].Message) ~= "BloodyHellMachine.ArmorMessage" || 
				 string(LocalMessages[i].Message) ~= "BloodyHellMachine.CatWomanMessage" || 
				 string(LocalMessages[i].Message) ~= "BloodyHellMachine.DrunkMessage" || 
				 string(LocalMessages[i].Message) ~= "BloodyHellMachine.GeoViewMessage" || 
				 string(LocalMessages[i].Message) ~= "BloodyHellMachine.GodModeMessage" || 
				 string(LocalMessages[i].Message) ~= "BloodyHellMachine.HatMessage" || 
				 string(LocalMessages[i].Message) ~= "BloodyHellMachine.HeadMessage" || 
				 string(LocalMessages[i].Message) ~= "BloodyHellMachine.HHGrenadeMessage" || 
				 string(LocalMessages[i].Message) ~= "BloodyHellMachine.InstaDeathMessage" || 
				 string(LocalMessages[i].Message) ~= "BloodyHellMachine.MaskMessage" || 
				 string(LocalMessages[i].Message) ~= "BloodyHellMachine.PipeSelfDestMessage" || 
				 string(LocalMessages[i].Message) ~= "BloodyHellMachine.RadarMessage" || 
				 string(LocalMessages[i].Message) ~= "BloodyHellMachine.StarmanMessage" || 
				 string(LocalMessages[i].Message) ~= "BloodyHellMachine.ToxicMessage" || 
				 string(LocalMessages[i].Message) ~= "BloodyHellMachine.UHHGrenadeMessage" || 
				 string(LocalMessages[i].Message) ~= "BloodyHellMachine.ZedDeathMessage" || 
				 string(LocalMessages[i].Message) ~= "BloodyHellMachine.ToxicMessage")
		{
			PosX = 0.325;
			DrawDamageMessage(C, i, PosX, PosY, DX, DY);
		...
		// Is this a countdown Msg?
		else if ( string(LocalMessages[i].Message) ~= "BloodyHellMachine.CountDownMessage" ||
				  string(LocalMessages[i].Message) ~= "SXKMachine.CountDownMessage" )
		{
			PosX = 0.4;
			DrawDamageMessage(C, i, PosX, PosY, DX, DY);
			return;
	...
	{
		LocalMessages[i].Message.static.RenderComplexMessage( C, LocalMessages[i].DX, LocalMessages[i].DY, class'Helper'.static.ParseTags(LocalMessages[i].StringMessage), LocalMessages[i].Switch, LocalMessages[i].RelatedPRI, LocalMessages[i].RelatedPRI2, LocalMessages[i].OptionalObject );
	}
	else
		C.DrawTextClipped( class'Helper'.static.ParseTags(LocalMessages[i].StringMessage), false );
...
simulated function DrawMessageB( Canvas C, int i, float PosX, float PosY, out float DX, out float DY )
{
    local float FadeValue;
	...
	if ( LocalMessages[i].Message.default.bComplexString )
	{
		LocalMessages[i].Message.static.RenderComplexMessage( C, LocalMessages[i].DX, LocalMessages[i].DY,
			 class'Helper'.static.ParseTags(LocalMessages[i].StringMessage), LocalMessages[i].Switch, LocalMessages[i].RelatedPRI,
...
simulated function DrawKillMessage( Canvas Canvas, int i, float PosX, float PosY, out float DX, out float DY )
{
	...
	S = class'Helper'.static.ParseTags(LocalMessages[i].StringMessage);
	...
	Canvas.DrawText(class'Helper'.static.ParseTags(S));
...
simulated function DrawDamageMessage( Canvas Canvas, int i, float PosX, float PosY, out float DX, out float DY )
{
	...
	Canvas.Font = KF2Font;//original kf2Fonts
...
function DrawFancyBar(Canvas Canvas)
{
	...
	local float BarWidth, DrawX, DrawY, WBW, LT; //BarHeight,
...
{	
 	local float Y, H, PatHP,PatHealingLevel, Ratio;
	local string PatHPText, PatSyringeCount, temp;
  	local ZombieBoss Boss;

  Y = 24.0 * (Canvas.ClipY / 1080.0);
  foreach DynamicActors(class'ZombieBoss', Boss)
  {
    PatHP = fclamp(Boss.Health/Boss.HealthMax,0,Boss.Health/Boss.HealthMax);
    PatHealingLevel = fclamp(Boss.HealingLevels[Boss.SyringeCount]/Boss.HealthMax,0,Boss.HealingLevels[Boss.SyringeCount]/Boss.HealthMax);
  	Ratio = Boss.Health / Boss.HealthMax;
  	temp = (Ratio*100) $ "%";	
	PatHPText = "Health: " $ Boss.Health $ "(" $ temp $ ")/" $ int(Boss.HealthMax);
	PatSyringeCount = string(int(Boss.SyringeCount));
    Canvas.SetDrawColor(255, 255, 255, 255);

    H = DrawSingleBoss(Boss.MenuName $ "        " $ PatHPText $ "HP        SyringeCountUsed:" $ PatSyringeCount, PatHP, PatHealingLevel, Canvas, Y);
    Y += H;
  }
}
	
function float DrawSingleBoss(string MonName, float Pct, float lvl, Canvas Canvas, float YStart)
{
	...
	// Draw the Pat healing level bar

	Canvas.SetPos(BL + Overlap, CenY);
	Canvas.SetDrawColor(255, 255, 255, 255);
	Canvas.DrawTile(BBMid, (BW - NewWidth - Overlap) * lvl, BH, 0, 0, BBMid.MaterialUSize(), BBMid.MaterialVSize());
...
function DrawPlayerInfo(Canvas C, Pawn P, float ScreenLocX, float ScreenLocY)
{
    ...
    DrawOverheadBar(C, P, ScreenLocX, ScreenLocY, fZoom, EnemyPRI, bSameTeam);
}
...
// DrawPlayerInfo
simulated function DrawOverheadBar( Canvas C, 
									Pawn P, 
									float ScreenLocX, 
									float ScreenLocY, 
									float fZoom, 
									...
									bool bSameTeam)
{
	...
	if ( ClassIsChildOf(EnemyPRI.ClientVeteranSkill,Class'KFVeterancyTypes') )
	{
		S = string(EnemyPRI.ClientVeteranSkillLevel) @ EnemyPRI.ClientVeteranSkill.default.VeterancyName;
		C.TextSize(S, XL, YL);
		TempY -= YL;
    ...
    C.DrawColor = class'HudBase'.default.WhiteColor;
	C.SetPos(LeftX, TempY);
	EnemySRPawn = KFHumanPawn(P);

	//--------------------------------
	// PERK
    //--------------------------------	
    if(EnemyPRI.ClientVeteranSkill != none)
	{
		...
		TempLevel = EnemyPRI.ClientVeteranSkillLevel;
...
simulated function DrawPortraitSE( Canvas Canvas )
{
	...
	StartX += (PortW * 0.5);
	SetFont(Canvas, 0.385);
	Canvas.Font = Font'MutKF2Hud.KF2CNF';
	...
	PortraitString = class'Helper'.static.ParseTags(PortraitString);;
	...
	// Text for the shadow
	ShadowString = class'Helper'.static.StripTags(PortraitString);
...
defaultproperties
{
     ...
     TinG=180
     TinB=255
     KF2Font=Font'MutKF2Hud.KF2CNF'
     KF2Digits=(DigitTexture=Texture'MutKF2Hud.Whiteness.KF2DigitTex',TextureCoords[0]=(X1=15,Y1=8,X2=68,Y2=82),TextureCoords[1]=(X1=89,Y1=8,X2=115,Y2=82),TextureCoords[2]=(X1=137,Y1=8,X2=192,Y2=82),TextureCoords[3]=(X1=210,Y1=8,X2=260,Y2=82),TextureCoords[4]=(X1=279,Y1=8,X2=331,Y2=82),TextureCoords[5]=(X1=353,Y1=8,X2=399,Y2=82),TextureCoords[6]=(X1=424,Y1=8,X2=470,Y2=82),TextureCoords[7]=(X1=494,Y1=8,X2=540,Y2=82),TextureCoords[8]=(X1=561,Y1=8,X2=610,Y2=82),TextureCoords[9]=(X1=631,Y1=8,X2=680,Y2=82),TextureCoords[10]=(X1=686,Y1=8,X2=726,Y2=82))
     KF2BLBase=(WidgetTexture=Texture'MutKF2Hud.KF2HUD.KF2BL_base',RenderStyle=STY_Alpha,TextureCoords=(X2=512,Y2=256),TextureScale=0.700000,PosX=0.015000,PosY=0.935000,ScaleMode=SM_Right,Scale=1.000000,Tints[0]=(B=255,G=255,R=255,A=215),Tints[1]=(B=255,G=255,R=255,A=215))
     KF2BLScan=(WidgetTexture=Texture'MutKF2Hud.KF2HUD.KF2BL_scanline',RenderStyle=STY_Alpha,TextureCoords=(X2=512,Y2=256),TextureScale=0.700000,PosX=0.015000,PosY=0.935000,ScaleMode=SM_Right,Scale=1.000000,Tints[0]=(B=255,G=255,R=255,A=255),Tints[1]=(B=255,G=255,R=255,A=255))
     KF2BLOverlay=(WidgetTexture=Texture'MutKF2Hud.KF2HUD.KF2BL_xpover',RenderStyle=STY_Alpha,TextureCoords=(X2=512,Y2=256),TextureScale=0.700000,PosX=0.015000,PosY=0.935000,ScaleMode=SM_Right,Scale=1.000000,Tints[0]=(B=255,G=255,R=255,A=255),Tints[1]=(B=255,G=255,R=255,A=255))
     KF2HealthIcon=(WidgetTexture=Texture'MutKF2Hud.KF2HUD.kf2ui_cross',RenderStyle=STY_Alpha,TextureCoords=(X2=512,Y2=256),TextureScale=0.100000,PosX=0.015000,PosY=0.935000,ScaleMode=SM_Right,Scale=1.000000,Tints[0]=(B=255,G=255,R=255,A=215),Tints[1]=(B=255,G=255,R=255,A=215))
     KF2ArmorIcon=(WidgetTexture=Texture'MutKF2Hud.KF2HUD.kf2ui_shield',RenderStyle=STY_Alpha,TextureCoords=(X2=512,Y2=256),TextureScale=0.100000,PosX=0.015000,PosY=0.935000,ScaleMode=SM_Right,Scale=1.000000,Tints[0]=(B=255,G=255,R=255,A=215),Tints[1]=(B=255,G=255,R=255,A=215))
     KF2SyringeIcon=(WidgetTexture=Texture'MutKF2Hud.KF2HUD.kf2ui_syringe',RenderStyle=STY_Alpha,TextureCoords=(X2=512,Y2=256),TextureScale=0.150000,PosX=0.015000,PosY=0.935000,ScaleMode=SM_Right,Scale=1.000000,Tints[0]=(B=255,G=255,R=255,A=255),Tints[1]=(B=255,G=255,R=255,A=215))
     KF2NadeIcon=(WidgetTexture=Texture'MutKF2Hud.KF2HUD.kf2ui_nade',RenderStyle=STY_Alpha,TextureCoords=(X2=512,Y2=256),TextureScale=0.100000,PosX=0.015000,PosY=0.935000,ScaleMode=SM_Right,Scale=1.000000,Tints[0]=(B=255,G=255,R=255,A=215),Tints[1]=(B=255,G=255,R=255,A=215))
     KF2BRBase=(WidgetTexture=Texture'MutKF2Hud.KF2HUD.KF2BR_base',RenderStyle=STY_Alpha,TextureCoords=(X2=512,Y2=256),TextureScale=0.700000,PosX=0.015000,PosY=0.935000,ScaleMode=SM_Right,Scale=1.000000,Tints[0]=(B=255,G=255,R=255,A=215),Tints[1]=(B=255,G=255,R=255,A=215))
     KF2BRScan=(WidgetTexture=Texture'MutKF2Hud.KF2HUD.KF2BR_scanline',RenderStyle=STY_Alpha,TextureCoords=(X2=512,Y2=256),TextureScale=0.700000,PosX=0.015000,PosY=0.935000,ScaleMode=SM_Right,Scale=1.000000,Tints[0]=(B=255,G=255,R=255,A=255),Tints[1]=(B=255,G=255,R=255,A=255))
     KF2BROverlay=(WidgetTexture=Texture'MutKF2Hud.KF2HUD.KF2BR_overlay',RenderStyle=STY_Alpha,TextureCoords=(X2=512,Y2=256),TextureScale=0.700000,PosX=0.015000,PosY=0.935000,ScaleMode=SM_Right,Scale=1.000000,Tints[0]=(B=255,G=255,R=255,A=255),Tints[1]=(B=255,G=255,R=255,A=255))
     KF2BRAngle=(WidgetTexture=Texture'MutKF2Hud.KF2HUD.KF2BR_angle',RenderStyle=STY_Alpha,TextureCoords=(X2=512,Y2=256),TextureScale=0.700000,PosX=0.015000,PosY=0.935000,ScaleMode=SM_Right,Scale=1.000000,Tints[0]=(B=255,G=255,R=255,A=255),Tints[1]=(B=255,G=255,R=255,A=255))
     KF2BRMelee=(WidgetTexture=Texture'MutKF2Hud.KF2HUD.KF2BR_melee',RenderStyle=STY_Alpha,TextureCoords=(X2=512,Y2=256),TextureScale=0.700000,PosX=0.015000,PosY=0.935000,ScaleMode=SM_Right,Scale=1.000000,Tints[0]=(B=255,G=255,R=255,A=255),Tints[1]=(B=255,G=255,R=255,A=255))
     KF2DoshIcon=(WidgetTexture=Texture'MutKF2Hud.KF2HUD.kf2ui_dosh',RenderStyle=STY_Alpha,TextureCoords=(X2=512,Y2=256),TextureScale=0.100000,PosX=0.015000,PosY=0.935000,ScaleMode=SM_Right,Scale=1.000000,Tints[0]=(B=255,G=255,R=255,A=215),Tints[1]=(B=255,G=255,R=255,A=215))
     KF2WeightIcon=(WidgetTexture=Texture'MutKF2Hud.KF2HUD.kf2ui_weight',RenderStyle=STY_Alpha,TextureCoords=(X2=512,Y2=256),TextureScale=0.100000,PosX=0.015000,PosY=0.935000,ScaleMode=SM_Right,Scale=1.000000,Tints[0]=(B=255,G=255,R=255,A=215),Tints[1]=(B=255,G=255,R=255,A=215))
     KF2BatteryIcon=(WidgetTexture=Texture'MutKF2Hud.KF2HUD.kf2ui_battery',RenderStyle=STY_Alpha,TextureCoords=(X2=512,Y2=256),TextureScale=0.150000,PosX=0.015000,PosY=0.935000,ScaleMode=SM_Right,Scale=1.000000,Tints[0]=(B=255,G=255,R=255,A=255),Tints[1]=(B=255,G=255,R=255,A=215))
     KF2AltBase=(WidgetTexture=Texture'MutKF2Hud.KF2HUD.KF2BR_basealt',RenderStyle=STY_Alpha,TextureCoords=(X2=512,Y2=256),TextureScale=0.700000,PosX=0.015000,PosY=0.935000,ScaleMode=SM_Right,Scale=1.000000,Tints[0]=(B=255,G=255,R=255,A=215),Tints[1]=(B=255,G=255,R=255,A=215))
     KF2AltScan=(WidgetTexture=Texture'MutKF2Hud.KF2HUD.KF2BR_scanlinealt',RenderStyle=STY_Alpha,TextureCoords=(X2=512,Y2=256),TextureScale=0.700000,PosX=0.015000,PosY=0.935000,ScaleMode=SM_Right,Scale=1.000000,Tints[0]=(B=255,G=255,R=255,A=255),Tints[1]=(B=255,G=255,R=255,A=255))
     ...
     ChatFontScale=0.52500000
     PickupFontScale=0.7500000
     KF2TLBase=(WidgetTexture=Texture'MutKF2Hud.KF2HUD.KF2TL_base',RenderStyle=STY_Alpha,TextureCoords=(X2=512,Y2=256),TextureScale=0.500000,PosX=0.015000,PosY=0.935000,ScaleMode=SM_Right,Scale=1.000000,Tints[0]=(B=255,G=255,R=255,A=215),Tints[1]=(B=255,G=255,R=255,A=215))
     KF2TLScan=(WidgetTexture=Texture'MutKF2Hud.KF2HUD.KF2TL_scanline',RenderStyle=STY_Alpha,TextureCoords=(X2=512,Y2=256),TextureScale=0.700000,PosX=0.015000,PosY=0.935000,ScaleMode=SM_Right,Scale=1.000000,Tints[0]=(B=255,G=255,R=255,A=255),Tints[1]=(B=255,G=255,R=255,A=215))
     KF2TLOverlay=(WidgetTexture=Texture'MutKF2Hud.KF2HUD.KF2TL_redline',RenderStyle=STY_Alpha,TextureCoords=(X2=512,Y2=256),TextureScale=0.700000,PosX=0.015000,PosY=0.935000,ScaleMode=SM_Right,Scale=1.000000,Tints[0]=(B=50,G=50,R=158,A=255),Tints[1]=(B=255,G=255,R=255,A=215))
     KF2TLSquare=(WidgetTexture=Texture'MutKF2Hud.KF2HUD.KF2TL_square',RenderStyle=STY_Alpha,TextureCoords=(X2=512,Y2=256),TextureScale=0.500000,PosX=0.015000,PosY=0.935000,ScaleMode=SM_Right,Scale=1.000000,Tints[0]=(B=255,G=255,R=255,A=192),Tints[1]=(B=255,G=255,R=255,A=215))
     KF2TLClock=(WidgetTexture=Texture'MutKF2Hud.KF2HUD.kf2ui_clock',RenderStyle=STY_Alpha,TextureCoords=(X2=512,Y2=256),TextureScale=0.100000,PosX=0.015000,PosY=0.935000,ScaleMode=SM_Right,Scale=1.000000,Tints[0]=(B=255,G=255,R=255,A=215),Tints[1]=(B=255,G=255,R=255,A=215))
     KF2TLZed=(WidgetTexture=Texture'MutKF2Hud.KF2HUD.kf2ui_zed',RenderStyle=STY_Alpha,TextureCoords=(X2=512,Y2=256),TextureScale=0.100000,PosX=0.015000,PosY=0.935000,ScaleMode=SM_Right,Scale=1.000000,Tints[0]=(B=255,G=255,R=255,A=215),Tints[1]=(B=255,G=255,R=255,A=215))
     ...
     ArrowScale=0.500000
     ArrowUp=Texture'MutKF2Hud.KF2HUD.KF2TL_arrowup'
     ArrowDown=Texture'MutKF2Hud.KF2HUD.KF2TL_arrowdown'
     ...
     KillDampen=0.850000
     KillBG=Texture'MutKF2Hud.KF2HUD.kf2ui_kill'
     SkullIcon=Texture'MutKF2Hud.KF2HUD.kf2ui_skull'
     ...
     WaveCompleteSound=Sound'MutKF2Hud.MusicStingers.kf2mid_wavecomplete'
     WaveBeginSound=Sound'MutKF2Hud.MusicStingers.kf2mid_waveincoming'
     MidWarnTex=Texture'MutKF2Hud.KF2Mid.kf2mid_caution'
     MidSolidTex=Texture'MutKF2Hud.KF2Mid.kf2mid_solid'
     MidLeftTex=Texture'MutKF2Hud.KF2Mid.kf2mid_left'
     MidRightTex=Texture'MutKF2Hud.KF2Mid.kf2mid_right'
     WaveIncomingString="W A V E I N C O M I N G"
     WaveCompleteString="W A V E C O M P L E T E"
     MidTextColor=(B=39,G=34,R=142)
     WaitingMessageClass="KFMod.WaitingMessage"
     BBLeft=Texture'MutKF2Hud.KF2Boss.kf2bb_left'
     BBMid=Texture'MutKF2Hud.KF2Boss.kf2bb_middle'
     BBRight=Texture'MutKF2Hud.KF2Boss.kf2bb_right'
     BBSkull=Texture'MutKF2Hud.KF2Boss.kf2bb_skull'
     ...
     InventoryBackgroundTexture=Texture'MutKF2Hud.KF2HUD.Hud2_Rectangel_W_Stroke'
     SelectedInventoryBackgroundTexture=Texture'MutKF2Hud.KF2HUD.Hud2_Rectangel_selected'
}
